...
Function generic_audio() As Object
	template = {
		title: "Generic audio template",
		description: "RSS to ORML audio",
		...
		itemType: "audio", ' the type of orml item that you want to generate
		limit: 20, ' limits the number of items retrieved from the original feed. -1 = no limit

		' where can the item array be found?
		' you do not need to specify the root element (e.g., "channel[0].item" instead of "rss[0].channel[0].item")
		' and you do not need to use dynamic tags here
